...
class Service {
    ...
    /**
     * Récupère la liste complète des comptes via le service SOAP.
    ...
    /**
     * Crée un nouveau compte bancaire via l'appel SOAP au serveur distant.
     * @param solde Montant initial du solde en DH.
     * @param compteType Type de compte (COURANT ou EPARGNE).
     * @return true si la création a réussi, false sinon.
     */
    fun createCompte(solde: Double, compteType: TypeCompte): Boolean {
        val request = SoapObject(NAMESPACE, METHOD_CREATE_COMPTE)
        
    // Conversion du Double en String obligatoire (ksoap2 incompatible avec les nombres décimaux)
    // Le serveur doit parser la chaîne pour reconstituer la valeur numérique
        request.addProperty("solde", solde.toString())
        request.addProperty("type", compteType.name)
    ...
    /**
     * Supprime un compte bancaire identifié par son numéro via la requête SOAP.
    ...
    fun deleteCompte(id: Long): Boolean {
        ...
        // Utilisation de PropertyInfo pour assurer la sérialisation correcte du type Long
